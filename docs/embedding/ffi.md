...
| `rift_delete_all` | `int rift_delete_all(RiftHandle* h)` | `0` on success, `-1` on error. Each imposter is torn down (as `rift_delete_imposter`) before this returns. If any imposter could not be deleted, the others still are, and the call returns `-1` with `rift_last_error` naming the ports that remain. |
