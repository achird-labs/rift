...
| W017 | A key the engine parses and does not act on: `_rift.metrics`, `_rift.proxy`, `recordMatches: true`, `_rift` on a `proxy`, `inject` or `fault` response, or a `_behaviors`/`behaviors` block setting anything besides `repeat` on a `fault` or `_rift`-only response. The engine reports the same keys as `config_key_ignored` in `_rift.warnings` | `"recordMatches": true` |
| W018 | A `behaviors` array element that sets more than one behavior (`wait`, `copy`, `lookup`, `decorate`, `shellTransform`). The engine runs them in its fixed order — the order the object form uses — not the order they were written in, which Mountebank uses. Put each behavior in its own element, in the order it should run. `repeat` does not count: it is not a step | `"behaviors": [{"decorate": "…", "copy": {…}}]` |
