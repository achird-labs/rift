...
| W017 | A key the engine parses and does not act on: `_rift.metrics`, `_rift.proxy`, `recordMatches: true`, `_rift` on a `proxy`, `inject` or `fault` response, or a non-empty `_behaviors`/`behaviors` block on a `proxy`, `inject`, `fault` or `_rift`-only response. The engine reports the same keys as `config_key_ignored` in `_rift.warnings` | `"recordMatches": true` |
