...
| `rift_script_execution_duration_ms` | histogram | `rule_id`, `result` | Script execution time, in milliseconds. `result` is `pass` (no fault), `fault` (the script decided a fault) or `error`. |
| `rift_script_errors_total` | counter | `rule_id`, `error_type` | Script failures. `error_type` is `timeout` (the script missed its deadline) or `runtime` (any other failure). |
| `rift_flow_state_ops_total` | counter | `operation`, `result` | Flow-store operations, by `operation` (`get`, `set`, `exists`, `delete`, `increment`, `increment_by`, `set_ttl`, `clear_flow`, `flow_ids`, `entry_count`) and `result` (`success`/`error`). |
| `rift_upstream_request_duration_ms` | histogram | `method`, `status` | Upstream request time for `proxy` responses, in milliseconds — the forward itself, not Rift's own handling. |
...
**P99 upstream latency (`proxy` responses):**
```promql
    ...
    rules:
      ...
      - alert: RiftScriptErrors
        ...
        annotations:
          ...
          description: "error_type={% raw %}{{ $labels.error_type }}{% endraw %}"
```
