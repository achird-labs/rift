...
Behaviors run on `is`, `inject` and `proxy` responses, and `repeat` on every response. A block
setting anything else on a `fault` or `_rift`-only response is kept and returned, and reported as
