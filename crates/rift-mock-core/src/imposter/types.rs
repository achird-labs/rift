...
pub enum StubResponse {
    ...
    Proxy {
        ...
        /// A behaviors block written on this response. No behavior runs on a `proxy` response, so
        /// it is kept only to be reported (`config_key_ignored`, issue #1181), to round-trip, and for
        /// the `--allowInjection` gate to classify. `None` for an absent or empty block.
    ...
    Inject {
        ...
        /// it is kept only to be reported (`config_key_ignored`, issue #1152) and to round-trip.
        ignored_rift: Option<RiftResponseExtension>,
        /// A behaviors block written on this response. No behavior runs on an `inject` response, so
        /// it is kept only to be reported (`config_key_ignored`, issue #1181), to round-trip, and for
        /// the `--allowInjection` gate to classify. `None` for an absent or empty block.
        ignored_behaviors: Option<serde_json::Value>,
    },
    Fault {
        ...
        /// it is kept only to be reported (`config_key_ignored`, issue #1152) and to round-trip.
        ignored_rift: Option<RiftResponseExtension>,
        /// A behaviors block written on this response. No behavior runs on a `fault` response, so
        /// it is kept only to be reported (`config_key_ignored`, issue #1181), to round-trip, and for
        /// the `--allowInjection` gate to classify. `None` for an absent or empty block.
        ignored_behaviors: Option<serde_json::Value>,
    },
    ...
    RiftScript {
        ...
        /// A behaviors block written beside a `_rift`-only response. No behavior runs on a script
        /// response, so it is kept only to be reported (`config_key_ignored`, issue #1181), to
        /// round-trip, and for the `--allowInjection` gate to classify. `None` for an absent or empty
        /// block.
        ignored_behaviors: Option<serde_json::Value>,
    },
}
