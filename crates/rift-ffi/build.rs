//! Stamp build identity (issue #344) into compile-time env vars that `rift_build_info`
//! (issue #343) reads via `option_env!`. Each value: an explicit env override wins (CI, source
//! tarballs), else it is derived here, else left unset so the field serializes as JSON `null`.
