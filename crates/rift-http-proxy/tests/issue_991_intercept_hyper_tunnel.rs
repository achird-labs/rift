...
// ===== Issue #996: HTTP/2 negotiates through the tunnel =====
//
// The listener pinned ALPN to `http/1.1`, so an SUT that speaks h2 to the real origin was silently
// downgraded when routed through the intercept proxy — it stopped exercising the protocol it uses
// in production, and an h2-only client got nothing. The imposter listener has auto-negotiated
// since #295; this brings the one TLS surface that did not.
//
// These assert on `response.version()` rather than on the body alone: a downgraded h2 client still
// gets a correct HTTP/1.1 answer, so body-only assertions pass just as happily without the fix.

// #996 AC1 (serve path).
#[tokio::test]
async fn an_h2_client_negotiates_http2_through_the_tunnel() {
    let server = start_intercept().await;
    let admin = server.admin_addr();
    let intercept = server.intercept_addr().expect("intercept bound");
    let ca = ca_pem(admin).await;
    add_rule(
        admin,
        r#"{"host":"cdn.example.com","action":{"serve":{"statusCode":200,"body":"served-over-h2"}}}"#,
    )
    .await;
    ...
    assert_eq!(
        ...
        "the tunnel must offer h2 in ALPN so a capable client negotiates it"
    );
    assert_eq!(response.text().await.expect("body"), "served-over-h2");
    server.shutdown().await;
}

// #996 AC1 (forward path). The hop to the imposter is plaintext HTTP/1.1 either way — what is
// asserted here is that negotiating h2 on the client side does not break the forward.
#[tokio::test]
async fn h2_negotiation_survives_a_forward_rule() {
    let server = start_intercept().await;
    let admin = server.admin_addr();
    let intercept = server.intercept_addr().expect("intercept bound");
    let ca = ca_pem(admin).await;
...
// #996 AC1 (no-rule-match path). The fall-through 200 is generated by a different function than
// the serve path, so it gets its own assertion.
#[tokio::test]
async fn the_no_rule_default_answers_over_h2() {
    let server = start_intercept().await;
    let admin = server.admin_addr();
    let intercept = server.intercept_addr().expect("intercept bound");
    let ca = ca_pem(admin).await;
    ...
    let response = proxy_client(intercept, &ca)
        .get("https://unconfigured.example.com/thing")
        .send()
        .await
        .expect("intercepted");
    ...
    assert!(
        response
            ...
            .await
            .expect("body")
            .contains("rift intercepted"),
        "the unconfigured-host fall-through must still answer over h2"
    );
    server.shutdown().await;
}

// #996 AC2. The control: advertising h2 must not break a client that only speaks HTTP/1.1. This
// passes before the fix too — it is here so a change that swapped h1 for h2 rather than offering
// both cannot satisfy the gate.
#[tokio::test]
async fn an_http1_only_client_is_unaffected() {
    let server = start_intercept().await;
    let admin = server.admin_addr();
    let intercept = server.intercept_addr().expect("intercept bound");
    let ca = ca_pem(admin).await;
    add_rule(
        admin,
        r#"{"host":"cdn.example.com","action":{"serve":{"statusCode":200,"body":"served-over-h1"}}}"#,
    )
    .await;
    ...
    assert_eq!(
        ...
        "an http/1.1-only client must still negotiate h1"
    );
    assert_eq!(response.text().await.expect("body"), "served-over-h1");
    server.shutdown().await;
}

// #996 AC4. Rule matching is above the protocol, so one rule must answer both. Asserted with the
// SAME rule and the same request in one test, rather than two tests that could drift apart.
#[tokio::test]
async fn the_same_rule_matches_over_both_protocols() {
    let server = start_intercept().await;
    let admin = server.admin_addr();
    let intercept = server.intercept_addr().expect("intercept bound");
    let ca = ca_pem(admin).await;
    add_rule(
        admin,
        r#"{"host":"cdn.example.com","predicates":[{"equals":{"path":"/only-this"}}],"action":{"serve":{"statusCode":201,"body":"protocol-agnostic"}}}"#,
    )
    .await;

    let h2 = proxy_client(intercept, &ca)
        .get("https://cdn.example.com/only-this")
        .send()
        .await
        .expect("h2 request");
    assert_eq!(h2.version(), reqwest::Version::HTTP_2);
    assert_eq!(h2.status(), 201);
    assert_eq!(h2.text().await.expect("body"), "protocol-agnostic");
    ...
    assert_eq!(h1.status(), 201);
    assert_eq!(h1.text().await.expect("body"), "protocol-agnostic");

    server.shutdown().await;
}

// #996 AC4, the case a `path` predicate cannot reach. HTTP/2 carries the authority in the
// `:authority` pseudo-header and hyper does not synthesize a `host` header from it, so a rule
// predicating on the `host` HEADER matched over h1 and silently stopped matching over h2 — the
// protocol-dependence AC4 exists to rule out. `the_same_rule_matches_over_both_protocols` uses a
// path predicate and sails past it, which is exactly why this one predicates on the header.
#[tokio::test]
async fn a_host_header_predicate_matches_over_both_protocols() {
    let server = start_intercept().await;
    let admin = server.admin_addr();
    let intercept = server.intercept_addr().expect("intercept bound");
    let ca = ca_pem(admin).await;
    add_rule(
        admin,
        r#"{"host":"cdn.example.com","predicates":[{"equals":{"headers":{"host":"cdn.example.com"}}}],"action":{"serve":{"statusCode":200,"body":"host-header-matched"}}}"#,
    )
    .await;
    ...
    assert_eq!(h2.version(), reqwest::Version::HTTP_2);
    assert_eq!(
        h2.text().await.expect("body"),
        "host-header-matched",
        "a host-header predicate must fire over h2, where the authority arrives as :authority"
    );
    ...
    let h1 = h1_client
        ...
        .expect("h1 request");
    assert_eq!(h1.version(), reqwest::Version::HTTP_11);
    assert_eq!(h1.text().await.expect("body"), "host-header-matched");
