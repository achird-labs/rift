...
// Issue #500: end-to-end handler coverage for the three script-failure response contracts. The
// equivalent parity tests already exist as integration binaries under crates/rift-http-proxy/tests/
// (issue_355/440/323/375), but CI's main job runs `cargo test --workspace --all-features --lib`,
// and `--lib` excludes every tests/*.rs binary — so those contracts have NO CI-run coverage. These
// live in the lib's #[cfg(test)] module so `--lib` exercises them. They drive the full handler via
// a real listener (ImposterManager + reqwest) and assert status/headers/body shape per contract.
#[cfg(all(test, feature = "javascript"))]
mod handler_error_response_tests {
    use super::*;
    ...
    // inject-error header. Any other matcher error keeps the 5xx backend mapping (not exercised here
    // since it needs a failing backend).
    #[tokio::test]
